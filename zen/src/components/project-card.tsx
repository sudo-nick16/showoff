...
const ProjectCard: NextPage<ProjectProps> = ({
  project,
  className,
  callback,
  setProjects,
  ...
  return (
    <>
      <div
        ...
        backdrop-blur-2xl bg-[#1F29371E] relative p-2 sm:p-4 flex gap-x-4 items-start rounded-2xl w-[22rem] ${className}`}
      >
        {authState.user?.username === project!.username && showControls && (
        ...
        <div className="flex flex-col py-1 justify-start items-start h-20 sm:h-28">
          <Link href={`/${project!.username}/${project!.slug}`}>
            <h1 className="font-semibold text-start text-base leading-tight line-clamp-1 sm:line-clamp-2">
              {project!.title}
            </h1>
          </Link>
          <p className="mt-2 mb-2 text-primary break-words text-start text-sm line-clamp-1 sm:line-clamp-2 leading-tight w-full">
            {project!.tagline}
          </p>
